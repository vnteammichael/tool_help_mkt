...
def init_param():
    ...
    parser.add_argument('-m','-mode',dest='mode',help='mode run', default='ads')
...
def main():
    ...
    if args.mode == 'ads':
        ...
        mysql.delete_data_by_range_day(table=config['table']['metric'],start=str(start_date),end=str(end_date))
    ...
    if args.mode == 'recharge':
    #update num recharge
        print(f'Update recharge file from {target_date}.xlsx')
        try:
